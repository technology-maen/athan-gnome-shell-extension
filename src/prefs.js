...
const AzanPrefsWidget = new GObject.Class({
    ...
    _get_tab_config: function () {
        ...
        calculation_page.add_combo('Madhab', 'madhab', [
        ...
        this.latitude_box = location_page.add_spin('Latitude', 'latitude', {
            lower: -90.0000,
            upper: 90.0000,
            step_increment: 0.0001
        });

        this.longitude_box = location_page.add_spin('Longitude', 'longitude', {
            lower: -180.0000,
            upper: 180.0000,
            step_increment: 0.0001
        });
        ...
        this.auto_location = location_page.add_boolean('Automatic location', 'auto-location', updateLocationState);

        this.auto_location.connect('state-set', updateLocationState);

        location_page.add_combo('Timezone', 'timezone', [
        ...
        this.time_format_12 = display_page.add_boolean('AM/PM time format', 'time-format-12');

        display_page.add_combo('Which times?', 'concise-list', [
            { 'title': 'All times', 'value': '0' },
            { 'title': 'Concise', 'value': '1' }
        ], 'string');

        calculation_page.add_range('Date adjustment', 'hijri-date-adjustment', {
            min: -2,
            max: 2,
            step: 1,
            mark_position: 0,
...
export default class ExamplePreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        window._settings = this.getSettings('org.gnome.shell.extensions.azan');
